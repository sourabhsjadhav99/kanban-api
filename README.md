...
- `GET /api/tasks/:id` - fetch single task (task details page)
...
| `GET` | `/api/tasks/:id` | None | `200` -> `{ "_id", "title", "description", "status", "createdAt", "updatedAt" }` | `404` -> `{ "message": "Task not found" }`, `500` -> `{ "message": "Failed to fetch task", "error": "..." }` |
...
## Data Flow (UI <-> API)
- UI fetches the board list with `GET /api/tasks`.
- Clicking a card opens a details page that requests `GET /api/tasks/:id`.
- Drag/drop persists state using `PATCH /api/tasks/:id/status`.
- Create/delete are done via `POST /api/tasks` and `DELETE /api/tasks/:id`.
